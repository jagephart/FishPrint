...
#a program to write N, P and edible portions to excel via merginf various datasets.
...
r<-db_n %>% group_by(species) %>% summarise(P_byC=mean(P,na.rm = TRUE),N_byC=mean(N,na.rm = TRUE),
  ...
  r_edible<-db_n %>% group_by(species)%>%filter(Parts=="f")%>%   #include only fillet parts for the edible portion calc
...
db_model_short[db_model_short=="NaN"]<-NA
write.xlsx(db_model_short,'LCA_full_data__short_compiled.xlsx', showNA=FALSE)
