...
5. Alon is updating feed ingredient N and P calc indicies
6. Alon checking built in N difference for live weight versus dry weight
7. Alon checking equation for via fat N and P method

From Alon:
I made the following minor changes to the code and committed them to github
1) changing the indices of feed N/P from Canadian feed file --> re-downloaded the data file and copied over the new indices
2) "cleaning" the DM /1 thing --> added this into the N and P by fat functions
3) and converting built_N and P to DM...so all the N and P in the nutrition columns for the three different methods should have similar values. From a quick glance this is currently more true for N now than P. --> I am not sure I know what this one means
...
# Load lca data
#_______________________________________________________________________________________________________________________#
lca <- clean.lca(LCA_data = read.csv("Data/LCA_compiled.csv", stringsAsFactors = FALSE))

#_______________________________________________________________________________________________________________________#
# Load feed FP data 
#_______________________________________________________________________________________________________________________#
# Units of footprint are per 1 kg (apart from energy, which has per L) FIX IT: Rob, is this correct?
feed_fp <- read.csv("Data/Feed_FP_raw.csv", stringsAsFactors = FALSE)
# Tidy FP data
...
fishNutrition <- read.csv("Data/FishGenus.csv", stringsAsFactors = FALSE)
...
lca <- lca %>%
  right_join(fishNutrition, by = "Scientific.Name")
...
feedNutrition <-clean_feedNutrition(feedNutrition_data = 
                                      read.csv("Data/United-States-Canadian-Tables-of-Feed-1982-pages-68-921-with_CrudeProtein.csv",
...
```{r}
lca$feed.ghg <- estimate.feedFP(LCA_data = lca, Feed_data = feed_fp, FP_option = "Carbon")
lca$feed.water <- estimate.feedFP(LCA_data = lca, Feed_data = feed_fp, FP_option = "Water")
lca$feed.N <- estimate.feedFP(LCA_data = lca, Feed_data = feed_fp, FP_option = "Nitrogen")
lca$feed.P <- estimate.feedFP(LCA_data = lca, Feed_data = feed_fp, FP_option = "Phosphorus")
lca$feed.land <- estimate.feedFP(LCA_data = lca, Feed_data = feed_fp, FP_option = "Land")
```
