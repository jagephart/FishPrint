...
1. Fill in feed ingredient FP and protein content values
 - Alon will use the terrestrial feed data to extract the N and P content directly. Will need to do sensitivity checks around the weighting of the feed ingredients into the feed component categories
 - Agrifootprint database (Rob has used in the past) --> not free and there are some issues; Agribalize; Nathan Peltier's not all salmon are created equal. Rob will look back studies in his database for sources
 
2. Factors to multiply protein content by to get N and P
 - Patrik can get factors with reference
 
3. Whole fish N and P content by species
 - Alon will talk to Zach about this; also talk to Christina about the modelled values
...
5. How to handle Energy_demand column in non-feed ghg function?
 - Patrik - is wood use for the two observations representative of large scale production? i.e., is it acceptable to ignore?
 
 
6. The biggest gap is harvest and yield, which is needed to get a per unit land FP, as well as a surface area for evaporative losses. Need to talk through land FP since the time dimension doesn't seem clear to me.  - Patrik look into after holiday

7. What units do we want the FP estimates in?
 - Input from anyone (we just need to pick something at some point to make sure everything is consistent)
 - L, ha, t CO2eq, /fish at farm gate

8. Need help/suggestions for evap rate by country - looks like it is available through this, but it is not straightforward to extract: <http://www.fao.org/aquastat/en/climate-info-tool/>. We will also need to determine how best to average over time and space. 
 - There are a lot of options here, but we might consider biasing values within some buffer zone for brackish production. Inland is harder because we probably don't expect too much production in arid areas/times or maybe cold areas/times. We don't want to go too far down the road of modelling where aquaculture occurs though, as this becomes very complicated very fast. One approach here is to model it with no geographical or temporal preference in the evaporative loss factor, but with a parameter that would allow us to shift the factor toward higher or lower evaporative losses. 
 - Kelvin is pulling these

9. Need factor for increasing evaporation losses for aerated ponds
 - Any suggestions for refs for this? 
...
4. Standardize units throughout
5. Add categories we need to add/standardize are: system type, fed/unfed, aerated/non-aearated, intensity level
