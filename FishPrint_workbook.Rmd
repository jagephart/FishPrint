...
2. Fill in feed ingredient FP and protein content values
 - Alon pulled the terrestrial feed data N and P content
...
5. Add categorical variables we need to add/standardize are: system type, fed/unfed, aerated/non-aearated, intensity level
...
fishNutrition <- fishNutrition %>%
  ...
  summarise(N = mean(N, na.rm = TRUE), P = mean(P, na.rm = TRUE), 
            ...
            N_built_in = mean(N_built_in, na.rm = TRUE), .groups = "drop") # FIX IT: Alon - is it correct to take the mean here? 
...
lca$feed.N <- estimate.feed.NP(LCA_data = lca, Feed_data = feedNutrition, FP = "N")
lca$feed.P <- estimate.feed.NP(LCA_data = lca, Feed_data = feedNutrition, FP = "P")

# Apply model to estimate N and P lost to the environment
# FIX IT: Start here with Alon on call
