...
# DECISION: Select allocation method here:
# Data (constants) for final foot print calculation 
fp_dat <- fp_clean %>%
  filter(Allocation == "Mass" & Impact.category == "Global warming potential") 

# ORDER: Animal, crop, FMFO, soy
fp_c_dat <- fp_dat %>%
  filter(impact_factor == "Carbon") %>%
  select(-FP) %>%
...
fp_p_dat <- fp_dat %>%
  filter(impact_factor == "Phosphorus") %>%
  select(-FP) %>%
  pivot_wider(names_from = Category, values_from = FP_val) %>%
  as.matrix() %>%
  c()
...
ggsave(filename = file.path(outdir, "plot_feed-footprint_carbon_sci-level.png"), width = 11, height = 8.5)
...
ggsave(filename = file.path(outdir, "plot_feed-footprint_nitrogen_sci-level.png"), width = 11, height = 8.5)
...
ggsave(filename = file.path(outdir, "plot_feed-footprint_phosphorus_sci-level.png"), width = 11, height = 8.5)
...
ggsave(filename = file.path(outdir, "plot_feed-footprint_land_sci-level.png"), width = 11, height = 8.5)
...
ggsave(filename = file.path(outdir, "plot_feed-footprint_water_sci-level.png"), width = 11, height = 8.5)
